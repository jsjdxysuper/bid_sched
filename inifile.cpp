...
string CIniFile::GetStrValue(string domain, string key)
{
...
int CIniFile::IniInit(IniFileData_TS *f_inifile)
{
...
//size new size
int CIniFile::IniFileBufferModify(long size, IniFileData_TS *f_inifile)
{
    if(size <= f_inifile->i_filebuffer_size)
        return 0;
    long newlen = (2 + (size/1024)) * 1024;//ÿ1-2k
...
void * CIniFile::IniOpenFile( const unsigned char mode)
{
	return IniOpenFile(m_cPath.c_str(),  mode);
}
/*******************************************************************************
 *   desc: open ini file
 *------------------------------------------------------------------------------
 *  param: char *psz_file       -- file to open
 *         unsigned char mode   -- mode == 0 ֻ  mode > 0 д
 *------------------------------------------------------------------------------
 * return:  0   -- file succefully opened
 *         -1   -- fail to open ini file
 *         -2   -- fail to read file to buffer
*******************************************************************************/
void* CIniFile::IniOpenFile(const char *psz_file, const unsigned char mode)
{
    struct stat   statbuf;
    IniFileData_TS	*f_inifile = NULL;

    f_inifile = (IniFileData_TS	*)malloc(sizeof(IniFileData_TS));
    if(f_inifile == NULL)
        return NULL;
    IniInit(f_inifile);
...
//ļ
int CIniFile::IniCleanFile(void *fpini)
{
    if(fpini == NULL)
        return -1;
...
*******************************************************************************/
int CIniFile::IniCloseFile(void *fpini)
{
    if(fpini == NULL)
        return -1;
    IniFileData_TS *f_inifile = (IniFileData_TS *)fpini;
...
int CIniFile::IniFileSizeGet(void *fpini)
{
    if(fpini == NULL)
        return 0;
    IniFileData_TS *f_inifile = (IniFileData_TS *)fpini;
 ...
 * return: 0     -- found
*******************************************************************************/
int CIniFile::IniSearchContentStart(const long i_position, IniFileData_TS *f_inifile)
{
    long  i = 0;
    long  i_temp = 0;
 ...
 * return: 0   -- found
 ******************************************************************************/
int CIniFile::IniSearchContentEnd(const long i_startpos, IniFileData_TS *f_inifile)
{
    long  i = 0;
    long  i_temp = 0;
 ...
 *         -1  -- section not found
*******************************************************************************/
int CIniFile::IniSearchSection(const char *psz_section, IniFileData_TS *f_inifile)
{
    long i = 0;
    int b_skip = -1;
...
*******************************************************************************/
int CIniFile::IniGetString(const char *psz_section, const char *psz_key, char *psz_value, int value_size, void *fpini)
{
    *psz_value = '\0';
    if(fpini == NULL)
        return -1;
            ...
            if(strncasecmp(f_inifile->sz_filebuffer + i, psz_key, strlen(psz_key)) == 0)
            {
                ...
                if(f_inifile->sz_filebuffer[i] == '=')
                {
                    ...
                    while(j < f_inifile->i_sc_endpos && f_inifile->sz_filebuffer[j] != '\n')
                        j++;
                    j--;
                    if(f_inifile->sz_filebuffer[j] == '\r')//dosʽҪǰ\r\nȥ
                        j--;
...
*******************************************************************************/
int CIniFile::IniSetString(const char *psz_section, const char *psz_key, const char *psz_value, void *fpini)
{
